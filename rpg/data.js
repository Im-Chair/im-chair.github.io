...
const BASE_MANA = 30;  // 基礎法力（再加 精神×2）——v377 起全職業統一啟用（魔符系統的前提）
...
/* 異常狀態 (§8)：毒燃吃目標最大生命% */
const DOT = {
  base: 0.015,        // 1–10 層每層 1.5% 最大生命(毒燃通用)
  poisonTail: 0.005,  // 毒 11–20 層每層 0.5%(僅毒專精職·盜賊)
...
const AFFIXES = {
  ...
  thorns:{n:'荊棘', fmt:v=>`受擊反彈 受到傷害的 ${v}%`, min:3, max:18, slots:'a'},
...
const ROLL_BANDS = {
  ...
  thorns:[[3,5],[5,8],[8,12],[12,18]],   // 改 %反傷後的裝備區間（起點值，可調）
...
/* 符文詞綴：值純由稀有度決定（與樓層/輪迴脫鉤）。mul=素質/上限型走乘法；其餘加進 sumAffix。
   thorns 待 3b（改全域 % 反傷）後再加入符文池。 */
const RUNE_AFFIX = {
  str:{band:'gen',mul:1}, int:{band:'gen',mul:1}, spi:{band:'gen',mul:1}, vit:{band:'gen',mul:1}, agi:{band:'gen',mul:1},
  hp:{band:'gen',mul:1}, mp:{band:'gen',mul:1},
  crit:{band:'gen'}, defr:{band:'gen'}, agile:{band:'gen'}, vamp:{band:'gen'},
  greed:{band:'gen'}, mend:{band:'gen'}, mregen:{band:'gen'}, dotdrain:{band:'gen'},
  ptouch:{band:'layers'}, btouch:{band:'layers'},
  thorns:{band:'thornsB'},
  bpyre:{band:'pyre'}, ppyre:{band:'pyre'},
};
const RUNE_BAND = {   // [普通, 精緻, 稀有, 傳說] 各 [min,max]
  gen:    [[1,5],[6,10],[11,15],[16,20]],   // 素質(乘)／率／吸血／貪婪等(加)
  layers: [[1,2],[2,3],[3,4],[4,5]],        // 淬毒/淬焰 層數
  thornsB:[[1,2],[2,3],[3,4],[4,5]],        // 荊棘 %反傷
  pyre:   [[1,15],[10,30],[25,40],[35,50]], // 劇毒/烈焰 毒燃增傷
};
...
/* 裝備圖示對照：檔案在 eq/<slug>.webp。武器看 wtype，護甲／飾品看名稱。
   飾品 6 個名字共用 4 張圖（戒指＝指環、護符＝骨墜＝吊墜），沒對到的自動回落 emoji。 */
const ITEM_ICON = {
  皮甲:'leather', 鎖甲:'chain', 法袍:'robe', 胸甲:'plate', 斗篷:'cloak', 鱗甲:'scale',
  戒指:'ring', 指環:'ring', 護符:'pendant', 骨墜:'pendant', 懷錶:'watch', 油燈:'lamp',
};
...
const REALMS = [
  {n:'淺穴',     i:'🕳️', bg:'realm0', from:1,  to:10,  intro:'潮濕的岩壁上還留著前人的火把痕跡。這裡死的人最多——因為來的人最多。'},
  {n:'哥布林聚落', i:'👺', bg:'realm1', from:11, to:20,  rule:'drain1', rt:'首回合行動點 −1', intro:'淺穴之後豁然開朗，是一座燒著篝火、擠滿棚屋的巨大洞穴——哥布林的聚落。骨頭串成的哨子在你踏入的瞬間就被吹響了。\n\n來不及站穩腳步——此域每場戰鬥首回合行動點 −1。'},
  {n:'無光通道', i:'⚫', bg:'realm2', from:21, to:30,  rule:'heal75', rt:'一切回復效果 −25%', intro:'聚落之後，通道忽然斷了光源。伸手不見五指，只能靠腳步聲跟牆壁的觸感往前走。\n\n連自己的傷口在哪都摸不清——此域一切回復效果 -25%。'},
  {n:'舊都', i:'🏚️', bg:'realm3', from:31, to:40,  rule:'dodge12', rt:'敵人 12% 機率閃避', intro:'通道盡頭亮起微光——一整座地底都市的廢墟，斷牆殘瓦間還留著誰住過的痕跡，只是不知道是人，還是別的什麼。\n\n熟悉這片瓦礫的不只你一個——此域敵人有 12% 機率閃避你的攻擊。'},
  {n:'熔獄', i:'🌋', bg:'realm4', from:41, to:50,  rule:'rage5', rt:'敵人第 5 回合起狂怒', intro:'舊都之後，路開始往下燒。空氣燙得像會咬人，遠處傳來岩漿翻滾的悶響——你終於摸到了深淵本身的溫度。\n\n熱得讓誰都撐不住理智——此域敵人第 5 回合起就陷入狂怒。'},
  {n:'淵底',     i:'🌑', bg:'realm5', from:51, to:99999, intro:'心臟之下還有東西。沒有人到過這裡——現在有了。'},
];

/* u：顯示單位。openBlessInfo 的「這趟合計」以前一律不帶單位，
   五個祝福有四個是百分比，於是 +20% 被印成 +20。單位掛在資料上，顯示端只負責串。

   n 是模板，`{v}` 由 blessText() 換成實際值。寫死數字會分岔：烹食給 hp:8、岩漿池給 hp:12，
   直接印 n 會三個入口都說「+15%」。數字只准存在 v（或呼叫端傳的 pv），字串一律內插。 */
const BLESSINGS = [
  {k:'str', v:5, u:'%', n:'力量的祝福：武器攻擊 +{v}%'},
  {k:'crit', v:10, u:'%', n:'銳利的祝福：爆擊率 +{v}%'},
  {k:'vamp', v:12, u:'%', n:'血契的祝福：攻擊吸血 {v}%'},
  {k:'plate', v:10, u:'%', n:'守勢的祝福：每場戰鬥開始獲得 {v}% 最大生命的格擋'},
  {k:'hp', v:15, u:'%', n:'堅韌的祝福：生命上限 +{v}%'},
];
/* 兩個「穩拿」事件的祝福強度，刻意低於表定值（BLESSINGS[].v）——它們不吃隨機、不付代價。
   數值只存在這裡：按鈕文字、事件文字都從 grantBless() 回傳的 n 取，不要再抄一次數字。 */
const BLESS_EV = {cook:8, lava:12};
/* 走專屬管線、不進 sumAffix 的祝福 key。
   str 以前是「力量 +N」→ sumAffix('str') → statTotal('str')，但全專案 statTotal('str')
   只有 mainStat() 一個消費端，主素質是 int 的法師／制魔師因此完全吃不到（v436 修）。
   改成武器攻擊乘率後由 blessWpnMult() 結算；若仍讓它流進 sumAffix 會再多加一份力量素質。

   hp 在 v455 加入：以前是死值 +15 → sumAffix('hp')，靠 blessMult() 隨層數放大，
   於是「顯示 +15、實際 +45」。改成最大生命乘率後由 blessHpMult() 結算，顯示即結算。 */
const BLESS_NOT_AFFIX = {str:1, hp:1};
...
const EV_HINTS = {shrine:'香灰味', gambler:'骰子聲', merchant:'燈籠的紅光', spring:'水聲', can:'金屬滾動聲',
  ...
  mine:'鎬敲石的回音', oldfire:'煙味', wreck:'車軸斷裂的聲響', wishwell:'銅錢落水聲',
  cyst:'樹皮下的蠕動', vein:'樹汁的甜味', confess:'壓低的呼吸聲', candle:'蠟味', bloodspring:'灼人的熱氣', calcified:'一動不動的人影', scroll:'紙頁翻動聲'};

const RATE_CAP = 70;   // 三率上限% (§5)
const VAMP_CAP = 60;   // 吸血彙總上限%
const MREGEN_CAP = 100; // 回法彙總上限%
const STAT_DIV = [[Infinity,3.5]]; // 素質→率 換算 (§5)：近線性，每 ~3.5 素質換 1% 率（~245 素質觸 70% 頂；早期≈原本、後期可觸頂）
...
const CYC_K    = [1.5, 4.3, 12.3];          // 輪迴裝備「樓層成長」倍率（≈0.79×CYC_MULT）：只乘樓層項、不乘常數與骰值——基礎值與詞綴同軸成長，換裝時刻每重約落在 37–44 層。無限段 ×2.3/重（怪物 ×2.86，落差=牆）
// BLESS_CYC 於 v455 刪除：五個祝福全改成百分比後沒有死值需要縮放，blessMult()／blessCyc() 一併移除。

/* 傳送（下潛起點）的唯一來源。改任何一項只動這裡。
   顯示（startOptions／openDivePicker）與扣款（tryStart／startRun）以前各乘一次，
   於是「選單寫 110、實際扣 132」這種事只要改費率就會發生。 */
const TP = {
  first:   11,   // 第一個傳送階
  step:    10,   // 每 10 層一階（11, 21, 31 …）
  origCap: 41,   // 本源封在 41：不讓玩家傳送過 50 層首領（core.js 記 cp、account.js 遷移也讀這個）
  cycCap:  91,   // 輪迴 I–III 封 100 層，所以最深傳送階是 91；無限（cycle ≥ 4）不封頂
  /* 傳送費 ＝ 樓層 × 係數。五個下潛入口各一檔，愈深的輪迴愈貴：
     高輪迴的碎銀收入本來就高，跳層的機會成本要跟著長，否則深輪迴等於免費傳送。
     本源與輪迴 I 刻意維持舊值（10／12），只有 II 之後才變貴。 */
  fee:  {orig:10, c1:12, c2:16, c3:22, inf:30},
  supply: 5,                 // 傳送出發時給的補給碎銀 ＝ 樓層 × 係數
};

const REST_HEAL_PCT = 0.30;    // 營火與繩降平台的休息回血 ＝ 最大生命 × 此值（兩處是同一個「休息」概念，共用一個值）
const WEAK_MUL = 0.75;         // 虛弱：造成的傷害 ×此值（玩家與敵人雙向適用；STATUS_INFO 的說明字串也內插這個）
const REFORGE_LOCK_ADD = 0.5;  // 重鑄每鎖定一條詞綴，費用 +此比例
const POT_MANA_PCT = 0.30;     // 治療藥水順帶回復的法力比例（說明文字與 toast 都內插這個，不要寫死 30）

/* 狂怒：從第 at 回合起，敵人每過一回合傷害遞增 growth（線性、無上限）。
   熔獄（rule='rage5'）提前到第 atRage5 回合。
   預告與結算都必須走 battle.js 的 rageMulOf()，不要在別處重寫這條式子。 */
const RAGE = {at:8, atRage5:5, growth:0.15};

/* 以下三個都是 50，但語意完全不同——哪天要把最終王改成每 60 層一隻，
   只有 FINAL_BOSS_EVERY 該動，另外兩個不該跟著動。 */
const ORIG_DEPTH       = 50;                 // 本源全長：走到底＝通關（認證 ✓ 與存檔遷移的深度上限都讀這個）
const FINAL_BOSS_EVERY = 50;                 // 每幾層一隻最終王（輪迴會重複遇到）
const RUNE_TIER        = {mid:50, high:100}; // 符文稀有度里程碑樓層

/* 門的大圖（ui/*.webp）；沒列到的門型（boss）沿用原本的 emoji＝該首領自己的圖示 */
/* 事件畫面圖示：以 emoji 當 key 查表，這樣 93 個 showEventScreen 呼叫點一個都不用改。
   沒列到的 emoji 一律照舊顯示。 */
const EV_IMG = {
  '🕯️':'door_rest', '🕯':'door_rest', '🔥':'door_rest',   // 營火／冷燭祭壇／烹食／淬火／先人的火堆
  '📦':'door_chest',                                      // 寶箱
  ...
  '🪢':'ev_rope',   // 繩降平台
  '🚪':'ev_door',   // 低語的門・密室
  '⛩️':'ev_shrine', '⛩':'ev_shrine',   // 無名神龕
  '⛏️':'ev_mine',   '⛏':'ev_mine',     // 崩塌的礦道
  '🛞':'ev_cart',   // 破裂的貨車
  '🪵':'ev_cyst',   // 樹囊
  '🌿':'ev_root',   // 裸露的樹根
  '🌋':'ev_lava',   // 岩漿池
  '🕳':'ev_confess','🕳️':'ev_confess', // 懺悔室
  '⛲':'ev_well',   // 幽光泉水
  '✨':'ev_crack',  // 發光的裂縫
  '🔄':'ev_cycle',  // 輪迴
