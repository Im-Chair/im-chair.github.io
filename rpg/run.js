...
function openDivePicker(){
  ...
  for(let c=1; c<=cu; c++){
    const label = c>=4 ? '無限' : '輪迴 '+'I'.repeat(c);
    html += `<button class="btn small${pendingMode===c?' primary':''}" onclick="setMode(${c})">${label}</button>`;
  }
  html += '</div>';
  // 模式說明
  if(pendingMode==='orig'){
...
function nextFloor(){
  if(R.cycle > 0 && R.cycle < 4 && R.floor >= 100){ cycleComplete(); return; }   // 輪迴 I–III 封 100；無限(cycle 4)不封頂
...
function viewMerchantGear(idx){   // 商人裝備:先跟身上比較再決定買不買
  const st = R.shop[idx]; if(!st || st.kind!=='gear'){ buyMerchant(idx); return; }
...
/* ===== 委託板（可接任務） ===== */
const MAX_ACTIVE = 2;
function romanCyc(c){ return c===0?'本源':(c>=4?'無限':'輪迴'+'I'.repeat(c)); }
...
function genBounty(band){
  ...
  let floor;
  if(band==='above')      floor = cf + 5 + Math.floor(Math.random()*11);
  else if(band==='below') floor = cf - 15 + Math.floor(Math.random()*11);
  else                    floor = cf - 5 + Math.floor(Math.random()*11);
  floor = Math.max(lo, Math.min(cap, floor));
  let target = 0;
  if(type==='boss'){   // 對齊到最近的 5 的倍數（樓層王在 5,10,15…）
    let bf = Math.round(floor/5)*5;
    bf = Math.max(Math.ceil(lo/5)*5, Math.min(Math.floor(cap/5)*5, bf));
    floor = bf || 10;
  }
  if(type==='streakkill') target = 5 + Math.floor(Math.random()*10);
  const typeDiff = BOUNTY_TYPE_DIFF[type] || 1;
  const floorRatio = Math.max(0.6, Math.min(1.5, floor/Math.max(1,cf)));   // 相對評定樓層
  const hard = typeDiff>=1.7 ? 1.5 : 1;
  const diff = floor * (mode? cycMult(mode):1) * hard;
  const r = Math.random();
  let reward;
  ...
  const gems = Math.max(1, Math.round(1.8 * typeDiff * floorRatio * (1 + mode*0.3)));   // 鑽石依難度
  return {mode, floor, target, type, reward, gems, band:band||'at', state:'offer'};
}
...
function finalizeBounties(){   // 只有活著回營（逃脫/通關）才把「達成」轉為可回報；死掉的達成作廢
  ...
  for(const b of G.bounties){ if(b.state==='active' && b.met) b.state='ready'; delete b.met; }
}
...
function backToCamp(){   // 每次下潛回來(逃脫/死亡/通關)自動刷新可接委託(offer);進行中/可回報保留
  ...
  renderCamp();   // renderCamp → ensureBounties 會補滿 offer
  showScreen('s-camp');
}
