...
function genBounty(band){
  ...
  const cap = mode===0 ? Math.min(50, Math.max(10, G.orig.cp+9)) : Math.min(100, Math.max(20, cd(mode).cp+14));   // 輪迴樓層上限 100，懸賞不得超過
...
function bountyProgress(kind){
  ...
  for(const b of G.bounties){
    ...
    if(ok && b.state==='active'){ b.state='ready'; toast('🎯 委託達成——回懸賞板回報領獎'); }   // 改手動回報,不自動完成
  }
}
